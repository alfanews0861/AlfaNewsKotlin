...
async function notifyReporter(reporterId, postId, headline, type, imageUrl, specificReason) {
    try {
        ...
        const toHumanEditorialReason = (rawReason) => {
            if (!rawReason || rawReason.trim().length === 0) {
                if (type === 'DUPLICATE')
                    return "ఈ వార్తాంశం గత కొన్ని గంటల్లో మీ మండలంలో ఇప్పటికే మన యాప్‌లో ప్రచురితమైంది.";
                if (type === 'YOUTUBE_POLICY_VIOLATION')
                    return "యూట్యూబ్ కమ్యూనిటీ నిబంధనల ప్రకారం తీవ్ర రక్తపాతం లేదా భయానక దృశ్యాలు కలిగిన వీడియోలను యూట్యూబ్‌లో ప్రసారం చేయడం నిషేధం. మన ఛానల్ భద్రత దృష్ట్యా ఈ వీడియో వార్తను నిలిపివేయడం జరిగింది.";
                if (type === 'POLICY_VIOLATION')
                    return "వార్తలోని అంశాలు మా ఎడిటోరియల్ మార్గదర్శకాలకు అనుగుణంగా లేనందున ప్రచురించలేకపోయాము.";
                if (type === 'INTERNAL_ERROR')
                    return "సర్వర్ లేదా నెట్‌వర్క్ అంతరాయం వల్ల ఈ వార్త ప్రచురణ తాత్కాలికంగా నిలిచింది.";
                return "";
            }
 ...
 */
async function performRestoreAllReporters() {
    ...
    for (const candId of candidateUserIds) {
        try {
            ...
            if (snapId)
                snapId.docs.forEach(d => newsDocsMap.set(d.id, d));
            if (snapName)
                snapName.docs.forEach(d => newsDocsMap.set(d.id, d));
            if (snapOrig)
                snapOrig.docs.forEach(d => newsDocsMap.set(d.id, d));
            if (userId !== candId) {
                ...
                if (snapUid)
                    snapUid.docs.forEach(d => newsDocsMap.set(d.id, d));
                if (snapUName)
                    snapUName.docs.forEach(d => newsDocsMap.set(d.id, d));
            }
            const newsDocs = Array.from(newsDocsMap.values());
            if (newsDocs.length === 0 && !userData)
                continue;
            ...
            if (!userData) {
                ...
                const repDistrict = sampleDoc.district || (sampleDoc.categories && sampleDoc.categories.find((c) => !c.includes("వార్త") && c !== sampleDoc.category)) || "";
            ...
            if (!appMandal || !appDistrict) {
                ...
                if (!appQuery.empty) {
                    ...
                    if (!appMandal)
                        appMandal = aData.mandal || aData.assignedMandal || aData.selectedMandal || "";
                    if (!appDistrict)
            ...
            if (!appDistrict && newsDocs.length > 0) {
                const sample = newsDocs[0].data() || {};
                appDistrict = sample.district || (sample.categories && sample.categories.find((c) => !c.includes("వార్త") && c !== sample.category)) || "";
            }
 ...
 */
exports.restoreAllDowngradedReporters = (0, https_1.onCall)(async (request) => {
...
async function checkMandalVacancy(district, mandal, excludeUserId) {
    ...
    if (!reporterQuery.empty) {
        const activeReporters = reporterQuery.docs.filter(doc => {
            ...
            if (repData.suspended === true || repData.previouslyDowngraded === true)
                return false;
            return true;
        });
 ...
 */
async function auditExistingReporterPerformance(reporterId, reporterName, district, mandal) {
    const now = new Date();
    ...
    try {
        ...
        if (userDoc.exists) {
            ...
            if (uData.lastPostTimestamp) {
                ...
                if (typeof lp.toDate === 'function')
                    latestPostDate = lp.toDate();
                else if (lp instanceof Date)
                    latestPostDate = lp;
                else if (typeof lp === 'number')
                    latestPostDate = new Date(lp > 1e11 ? lp : lp * 1000);
            }
        }
    }
    catch (e) {
        ...
        for (const field of queryFields) {
            ...
            if (snap && !snap.empty) {
                for (const doc of snap.docs) {
                    ...
                    const data = doc.data();
                    if (data.status === 'rejected' || data.isPostRejected === true)
                        continue;
                    totalPostsThisMonth++;
                    const postLocation = (data.location || data.mandal || data.assignedMandal || "").trim();
                    const postDistrict = (data.district || "").trim();
                    const isOwnMandal = (0, location_data_1.areMandalsMatching)(postLocation, mandal, district || postDistrict) ||
 ...
 */
async function alertExistingReporterOfChallenger(existingReporterId, existingReporterName, district, mandal, applicantName, audit) {
 ...
 */
async function sendReporterApplicationEmail(data, shouldAutoApprove, isPreviouslyDowngraded, finalStatus, conflictInfo, performanceAudit) {
...
exports.submitReporterApplication = (0, https_1.onCall)({ secrets: ["EMAIL_USER", "EMAIL_PASS"] }, async (request) => {
    ...
    if (!finalFullName || finalFullName.length < 3) {
        throw new https_1.HttpsError('invalid-argument', 'కనీసం 3 అక్షరాలతో పూర్తి పేరు నమోదు చేయాలి.');
    }
    if (!finalFatherName || finalFatherName.length < 3) {
        throw new https_1.HttpsError('invalid-argument', 'కనీసం 3 అక్షరాలతో తండ్రి పేరు నమోదు చేయాలి.');
    }
    if (clean10.length !== 10 || !/^[6-9]\d{9}$/.test(clean10)) {
        throw new https_1.HttpsError('invalid-argument', 'చెల్లుబాటు అయ్యే 10 అంకెల మొబైల్ నంబర్ తప్పనిసరి (6-9 తో ప్రారంభం కావాలి).');
    }
    if (!finalAddress || finalAddress.length < 6) {
        throw new https_1.HttpsError('invalid-argument', 'కనీసం 6 అక్షరాలతో చిరునామా నమోదు చేయాలి.');
    }
    ...
    if (!trimmedDistrict || !trimmedMandal) {
        throw new https_1.HttpsError('invalid-argument', 'జిల్లా మరియు మండలం తప్పనిసరిగా ఎంచుకోవాలి.');
    }
    if (!finalInterestedArea) {
        throw new https_1.HttpsError('invalid-argument', 'ఆసక్తి ఉన్న కేటగిరీ తప్పనిసరి.');
    }
    if (!finalEducation) {
        throw new https_1.HttpsError('invalid-argument', 'విద్యార్హత తప్పనిసరి.');
    }
    if (!finalCurrentOrg) {
        throw new https_1.HttpsError('invalid-argument', 'ప్రస్తుత సంస్థ లేదా వృత్తి తప్పనిసరి.');
    }
    ...
    if (shouldAutoApprove && userId) {
        ...
        let perfAudit;
        try {
            perfAudit = await auditExistingReporterPerformance(existingRep.id, existingRep.name, trimmedDistrict, trimmedMandal);
        ...
        await sendReporterApplicationEmail({
            ...
            mandal: trimmedMandal,
            userId
        }, shouldAutoApprove, isPreviouslyDowngraded, finalStatus, { isConflict, existingReporterName: existingRep.name, existingReporterPhone: existingRep.phone }, perfAudit);
        return {
            success: true,
            autoApproved: shouldAutoApprove,
            isPreviouslyDowngraded,
    ...
    await sendReporterApplicationEmail({
        ...
        mandal: trimmedMandal,
        userId
    }, shouldAutoApprove, isPreviouslyDowngraded, finalStatus, { isConflict, existingReporterName: existingRep?.name, existingReporterPhone: existingRep?.phone });
    return {
        success: true,
        autoApproved: shouldAutoApprove,
        isPreviouslyDowngraded,
...
exports.onReporterApplicationCreated = (0, firestore_1.onDocumentCreated)({
    ...
    const isApplicationComplete = Boolean(applicantName.length >= 3 &&
    ...
    if (canAutoApprove) {
        ...
        await sendReporterApplicationEmail({ ...data, district, mandal, fullName: applicantName, phone: rawPhone, userId }, false, isPreviouslyDowngraded, "PENDING", { isConflict: true, existingReporterName: existingRep?.name, existingReporterPhone: existingRep?.phone }, perfAudit);
    }
    else {
...
async function getAssignedReporter(district, mandalam) {
    try {
        ...
        for (const doc of distReporters.docs) {
            ...
            if (repMandal && (0, location_data_1.areMandalsMatching)(mandalam, repMandal, district)) {
                return {
                    id: doc.id,
                    name: data.name || "Reporter"
                };
