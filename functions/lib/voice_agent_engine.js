 ...
 */
function generateInitialGreeting(callType, context) {
    ...
    switch (callType) {
        case types_1.VoiceCallType.INACTIVITY_FOLLOWUP:
        ...
        case types_1.VoiceCallType.EVENT_CAMPAIGN:
        ...
        case types_1.VoiceCallType.INBOUND_SUPPORT:
        default:
 ...
 */
async function processVoiceCallTurn(callType, context, history, userSpokenText) {
    const schema = {
        ...
        properties: {
            ...
            extractedIntent: {
                type: genai_1.Type.OBJECT,
                properties: {
                    reasonForInactivity: { type: genai_1.Type.STRING, description: "Identified reason: e.g. Power cut, Sickness, Traveling, Busy, App bug, No news, or none" },
                    promisedSubmissionTime: { type: genai_1.Type.STRING, description: "Promised submission time if stated, e.g. Today 5 PM, Tomorrow morning" },
                    followUpRequired: { type: genai_1.Type.BOOLEAN, description: "Whether desk needs to follow up later" },
                    wantsAdKit: { type: genai_1.Type.BOOLEAN, description: "True if user expressed interest in receiving ad tariff / ad collection details" },
                    adLeadContact: { type: genai_1.Type.STRING, description: "Any advertiser name or contact phone mentioned by reporter" },
                    sentiment: { type: genai_1.Type.STRING, enum: ["positive", "neutral", "negative"] },
                    notes: { type: genai_1.Type.STRING, description: "Brief 1-line note for the editorial team" }
                },
...
దయచేసి పై నియమాల ప్రకారం సరైన సమాధానం మరియు ఉద్దేశాన్ని (JSON ఫార్మాట్ లో) ఇవ్వండి.`;
    try {
        const result = await (0, utils_1.runWithAIFallback)(async (ai, modelName) => {
            ...
            const parsed = (0, utils_1.parseAIJson)(response);
            return parsed;
        });
