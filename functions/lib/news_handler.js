...
exports.recoverFalselyRejectedNewsHttp = exports.reprocessUnprocessedNewsHttp = exports.scheduleReprocessFailedReporterNews = exports.onNewsPostCreated = exports.processNewsPost = void 0;
...
exports.runReprocessFailedAndUnprocessedNews = runReprocessFailedAndUnprocessedNews;
 ...
 */
async function deleteOriginalFile(url) {
 ...
 */
function isApprovalRemark(reason) {
    if (!reason)
        return false;
...
function normalizeSingleStory(aiRes, actualPostData) {
    ...
    if (finalFullStoryTe && typeof finalFullStoryTe === 'string' && finalFullStoryTe.trim().length > 0) {
        const rawStory = (0, utils_1.sanitizeTeluguText)(finalFullStoryTe).trim();
        const storyWords = rawStory.split(/\s+/).filter(Boolean).length;
        if (storyWords >= 80 && rawStory !== finalContent) {
            finalFullStoryTe = (0, utils_1.formatIntoParagraphs)(rawStory);
        }
        else {
            finalFullStoryTe = "";
        }
    ...
    if (finalFullStoryEn && typeof finalFullStoryEn === 'string' && finalFullStoryEn.trim().length > 0) {
        ...
        if (storyWordsEn >= 80 && rawStoryEn !== finalContentEn) {
            finalFullStoryEn = (0, utils_1.formatIntoParagraphs)(rawStoryEn);
        }
        else {
            finalFullStoryEn = "";
        }
...
async function performAIProcessing(headline, content, actualPostData, recentStories = []) {
    ...
    const singleStorySchema = {
        ...
        properties: {
            ...
            fullStoryTe: { type: genai_1.Type.STRING, description: "Senior Editor full story in Telugu across 3-4 paragraphs separated by \\n\\n if source has 120+ words, or empty string \"\" if source is brief (< 120 words)" },
    ...
    if (data.aiProcessed && (status === "PROCESSING_VIDEO" || status === "PENDING_YOUTUBE_RETRY") && !data.videoProcessed) {
        ...
        try {
            ...
            if (organizations.length > 0)
                description += `సంస్థలు: ${organizations.join(', ')}\n`;
            if (locations.length > 0)
                description += `ప్రాంతాలు: ${locations.join(', ')}\n`;
            if (data.location)
                description += `స్థలం: ${data.location}\n`;
            description += `\n${hashTags}\n\n`;
 ...
 */
async function runReprocessFailedAndUnprocessedNews(hoursLookback = 24) {
    ...
    try {
        ...
        const retryCandidates = snapshot.docs.filter(doc => {
            ...
            if (d.approved === true && d.aiProcessed === true)
                return false;
 ...
 */
exports.reprocessUnprocessedNewsHttp = (0, https_1.onRequest)({
    region: utils_1.REGION,
    memory: "1GiB",
    timeoutSeconds: 300
}, async (req, res) => {
 ...
 */
exports.recoverFalselyRejectedNewsHttp = (0, https_1.onRequest)({ region: utils_1.REGION }, async (req, res) => {
    try {
