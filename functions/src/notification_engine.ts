...
export const sendPersonalizedNotification = onSchedule({
    ...
    if (istHour === 21 && overallBestNews) {
        try {
            logger.log("9 PM Slot: Sending Top 5 News to everyone via topic.");
        ...
        while (hasMoreUsers && totalUsersProcessed < maxUsersPerCategory) {
            ...
            usersSnapshot.docs.forEach(doc => {
                ...
                if (Array.isArray(user.fcmTokens)) {
                    user.fcmTokens.forEach((t: string) => { if (t && !tokens.includes(t)) tokens.push(t); });
                }
            ...
            if (usersSnapshot.docs.length < 500) hasMoreUsers = false;
            else startAfterDoc = usersSnapshot.docs[usersSnapshot.docs.length - 1];
        }
    ...
    try {
        ...
        if (!guestsSnapshot.empty && overallBestNews) {
            ...
            logger.log(`[NOTIF] Added ${guestsSnapshot.size} guest devices to delivery queue.`);
        }
    } catch (e) { logger.error("[NOTIF] Guest delivery error", e); }
