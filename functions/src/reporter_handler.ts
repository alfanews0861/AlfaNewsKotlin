...
export async function notifyReporter(
    ...
    try {
        ...
        if (userData && (userData.notificationsEnabled === false || userData.pushEnabled === false)) return;
 ...
 */
export async function executeCleanDuplicateApplications(): Promise<{ deletedCount: number }> {
 ...
 */
export const cleanDuplicateApplications = onCall(async (request) => {
    const auth = request.auth;
    if (!auth || !auth.uid) {
        throw new HttpsError('unauthenticated', 'మీరు లాగిన్ అవ్వాలి.');
    }
 ...
 */
export async function executeCleanupExpiredApplications(): Promise<{ deletedCount: number }> {
 ...
 */
export const cleanupExpiredReporterApplications = onSchedule({
    schedule: "0 2 * * *",
