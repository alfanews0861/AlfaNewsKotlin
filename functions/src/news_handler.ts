...
function normalizeSingleStory(aiRes: any, actualPostData: any): any {
    ...
    finalHeadline = cleanTeluguHeadline(finalHeadline);
    ...
    if (finalNotificationTitle.toLowerCase() === "null" ||
        ...
        finalNotificationTitle = cleanTeluguHeadline(finalNotificationTitle);
    }
 ...
 */
export async function extractVideoAudioSpeechContext(videoUrl: string, postId: string): Promise<string> {
        ...
        if (!fs.existsSync(videoLocalPath) || fs.statSync(videoLocalPath).size === 0) {
            ...
            if (!fs.existsSync(videoLocalPath) || fs.statSync(videoLocalPath).size === 0) {
                ...
                if (typeof (videoRes.body as any)[Symbol.asyncIterator] === 'function') {
                    ...
                    await pipeline(Readable.fromWeb(videoRes.body as any), fs.createWriteStream(videoLocalPath));
                }
            }
        }
...
ఒకవేళ ఆడియోలో ఎలాంటి మాటలు లేకపోతే (కేవలం నేపథ్య శబ్దం/ట్రాఫిక్/మ్యూజిక్ ఉంటే లేదా అర్థం కాని శబ్దాలుంటే) కేవలం "NO_SPEECH" అని మాత్రమే సమాధానం ఇవ్వండి.`;
        ...
        const speechResult = await runWithAIFallback(async (ai, modelName) => {
            const timeoutPromise = new Promise<null>((_, reject) =>
                setTimeout(() => reject(new Error("Audio AI timeout")), 15000)
            );
...
export async function performAIProcessing(
    ...
    const videoUrl = (isVideoPost && (rawMediaUrl || rawMediaUrls.find((u: string) => typeof u === 'string' && (u.includes('.mp4') || u.includes('video'))))) || "";
...
export const onNewsPostCreated = onDocumentWritten({
    ...
    if (data.aiProcessed && (status === "PROCESSING_VIDEO" || status === "PENDING_YOUTUBE_RETRY") && !data.videoProcessed) {
        ...
        if (data.isSafeForYouTube === false || latestData.isSafeForYouTube === false) {
            ...
            if (originalReporterId) {
                await notifyReporter(
                    ...
                    'YOUTUBE_POLICY_VIOLATION' as any,
                    "",
                    ytRejectionReason
                );
            }
        ...
        try {
            ...
            const videoSafety = await scanVideoSafetyWithGeminiAI(videoPath);
            let appliedBlurRanges: Array<{ start: number; end: number }> = [];
