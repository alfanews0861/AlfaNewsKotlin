...
async function syncTelanganaOfficialFeeds() {
    console.log("==================================================");
    console.log("🚀 TELANGANA OFFICIAL FEEDS (COLLECTORS, SPs, DPROs) SYNC");
    console.log("==================================================\n");
    ...
    const feedsPath = path.join(__dirname, '..', 'telangana_official_feeds.json');
    if (!fs.existsSync(feedsPath)) {
        console.error("❌ telangana_official_feeds.json not found!");
        process.exit(1);
    }
    const feeds = JSON.parse(fs.readFileSync(feedsPath, 'utf8'));
    console.log(`Loaded ${feeds.length} Telangana official administration feeds.\n`);
    ...
    for (let i = 0; i < feeds.length; i++) {
        ...
        if (existingMap.has(cleanHandle)) {
            ...
            const needsUpdate = (item.district && existing.district !== item.district) || 
                                ...
                                (!existing.category || existing.category === 'రాజకీయం'); // police/collectors shouldn't be tagged purely as political if they are official
    ...
    console.log("\n==================================================");
    console.log("🎉 TELANGANA OFFICIAL FEEDS SYNC COMPLETED");
...
syncTelanganaOfficialFeeds().catch(err => {
