...
async function syncAndhraVerifiedFeeds() {
    console.log("==================================================");
    console.log("🚀 SYNCING 100% LIVE-VERIFIED ANDHRA PRADESH FEEDS TO FIRESTORE");
    console.log("==================================================\n");
    ...
    const feedsPath = path.join(__dirname, '..', 'andhra_final_verified_feeds.json');
    if (!fs.existsSync(feedsPath)) {
        console.error("❌ andhra_final_verified_feeds.json not found!");
        process.exit(1);
    }
    const feeds = JSON.parse(fs.readFileSync(feedsPath, 'utf8'));
    console.log(`Loaded ${feeds.length} verified AP feeds to sync.\n`);

    // 1. Fetch current existing feeds to ensure NO duplicate insertion
    ...
    console.log("\n==================================================");
    console.log(`🎉 ANDHRA PRADESH SYNC COMPLETE!`);
...
syncAndhraVerifiedFeeds().catch(err => {
