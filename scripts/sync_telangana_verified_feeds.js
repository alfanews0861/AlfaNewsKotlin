...
async function syncVerifiedFeeds() {
    console.log("==================================================");
    console.log("🚀 SYNCING 100% LIVE-VERIFIED TELANGANA FEEDS TO FIRESTORE");
    console.log("==================================================\n");
    ...
    const feedsPath = path.join(__dirname, '..', 'telangana_final_verified_feeds.json');
    if (!fs.existsSync(feedsPath)) {
        console.error("❌ telangana_final_verified_feeds.json not found!");
        process.exit(1);
    }
    const feeds = JSON.parse(fs.readFileSync(feedsPath, 'utf8'));
    console.log(`Loaded ${feeds.length} verified feeds to sync.\n`);
