...
const ApifyIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12 0c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm0 2c5.523 0 10 4.477 10 10s-4.477 10-10 10-10-4.477-10-10 4.477-10 10-10zm0 3c-3.866 0-7 3.134-7 7s3.134 7 7 7 7-3.134 7-7-3.134-7-7-7zm0 2c2.761 0 5 2.239 5 5s-2.239 5-5 5-5-2.239-5-5 2.239-5 5-5zm0 1c-2.209 0-4 1.791-4 4s1.791 4 4 4 4-1.791 4-4-1.791-4-4-4z"/></svg>;
...
const ApifyFacebookScrapingPage: React.FC = () => {
    ...
    const fetchFeeds = useCallback(async () => {
        setIsFetching(true);
        try {
            const q = query(collection(db, 'apify_facebook_feeds'), orderBy('sourceName'));
    ...
    const handleSubmit = async (e: React.FormEvent) => { 
        ...
        try { 
            ...
            if (editingId) await updateDoc(doc(db, 'apify_facebook_feeds', editingId), payload); 
            else await addDoc(collection(db, 'apify_facebook_feeds'), { 
                ...payload, 
                lastStatus: 'active', 
                lastFetchTime: null, 
                isPaused: false,
    ...
    const handleManualTrigger = async () => {
        if (!window.confirm(`అడ్వాన్స్‌డ్ ఫేస్బుక్ స్క్రాపింగ్‌ను ప్రారంభించాలా? (ఇది ఇప్పుడు ఉచితం)`)) return;
        setIsProcessing(true);
        setStatusLog(['ఫేస్బుక్ స్కానింగ్ ప్రారంభమైంది...']);
        try {
            const processFn = httpsCallable(getFunctions(app, 'asia-south1'), 'processApifyFacebookFeeds');
        ...
        <div className="font-mallanna text-black animate-fade-in">
            <div className="bg-orange-600 p-6 rounded-[2rem] mb-8 flex flex-col md:flex-row justify-between items-center shadow-xl shadow-orange-100 gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-3xl font-ramabhadra text-white leading-tight">Advanced Facebook Scraper</h2>
                        <p className="text-orange-100 text-sm font-bold uppercase tracking-widest opacity-80">ఇంటర్నల్ మానిటరింగ్ డ్యాష్‌బోర్డ్ (Free)</p>
                    </div>
                </div>
                <button onClick={handleManualTrigger} disabled={isProcessing} className="bg-white text-orange-600 px-10 py-3 rounded-2xl font-bold text-xl shadow-lg active:scale-95 transition-all flex items-center gap-2">
                    ...
                    {isProcessing ? 'స్కానింగ్...' : 'Scrape Now (Free)'}
                </button>
            </div>

            {statusLog.length > 0 && (
                <div className="mb-8 p-4 bg-black rounded-[1.5rem] text-orange-400 font-mono text-xs max-h-40 overflow-y-auto shadow-inner border border-gray-800">
            ...
            <form onSubmit={handleSubmit} className="bg-white p-6 rounded-[2rem] shadow-sm border space-y-4 mb-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                        ...
                        <input type="text" value={sourceName} onChange={e => setSourceName(e.target.value)} placeholder="eg: AP News Page" className="w-full border p-4 rounded-2xl text-lg outline-none focus:ring-2 focus:ring-orange-500 bg-gray-50 transition-all font-semibold" required />
                    </div>
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-gray-400 uppercase tracking-widest ml-2">Page ID / Username</label>
                        <input type="text" value={url} onChange={e => setUrl(e.target.value)} placeholder="eg: apnews" className="w-full border p-4 rounded-2xl text-lg outline-none focus:ring-2 focus:ring-orange-500 bg-gray-50 transition-all font-semibold" required />
                    </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                ...
                {category === 'స్థానిక' && stateSelection && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1">
                            ...
                            <input type="text" value={mandal} onChange={e => setMandal(e.target.value)} placeholder="మండలం పేరు" className="w-full border p-4 rounded-2xl text-lg outline-none focus:ring-2 focus:ring-orange-500 bg-gray-50 transition-all font-semibold" required />
                        </div>
                    </div>
                )}
            ...
            <div className="space-y-4 pb-24">
                ...
                {isFetching && feeds.length === 0 ? (
                    ...
                    <div key={feed.id} className="bg-white p-5 rounded-[2rem] border shadow-sm hover:border-orange-200 transition-all group">
                        <div className="flex flex-col sm:flex-row gap-4">
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <span className="text-[10px] bg-orange-50 text-orange-700 px-2.5 py-1 rounded-full font-black uppercase tracking-wider">{feed.category}</span>
                                ...
                                <div className="grid grid-cols-4 gap-2">
                                    <StatItem label="నేటివి (Today)" value={feed.todayProcessedCount || 0} color="text-orange-600" />
                            ...
                            <div className="flex sm:flex-col gap-2 justify-center shrink-0 border-t sm:border-t-0 sm:border-l border-gray-100 pt-4 sm:pt-0 sm:pl-4">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    }} className="p-4 bg-orange-50 rounded-2xl text-orange-600 active:scale-90 transition-all"><EditIcon /></button>
                                    <button onClick={async () => { if (window.confirm("ఈ సోర్స్‌ను తొలగించాలా?")) { await deleteDoc(doc(db, 'apify_facebook_feeds', feed.id)); fetchFeeds(); } }} className="p-4 bg-red-50 rounded-2xl text-red-600 active:scale-90 transition-all"><DeleteIcon /></button>
                                </div>
                            </div>
                        </div>
...
export default ApifyFacebookScrapingPage;
